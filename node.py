...
class Model:
    ...
    def __init__(self, settings: Settings):
        ...
        self.roles = {var: self.col_data[self.col_data['var_name'] == var].var_role.values[0]
                      for var in self.split_vars}
        ...
        match self.col_data[self.col_data.var_name == col]['var_role'].iloc[0]:
            ...
            case 'S':
                ...
                for idx, cut in enumerate(cutpoints):
                    ...
                    sse_tab = pd.concat([sse_tab,
                                         pd.DataFrame({'cut': cut, 'sse': cut_sse}, index=[idx])])
        ...
        for col in self.split_vars:
            match self.col_data[self.col_data.var_name == col]['var_role'].iloc[0]:
                ...
                case 'c':
                    ...
                    indexes_by_value = self.df.loc[node.idx].groupby(col, dropna=False).\
    ...
    def _get_best_variable(self, node) -> str:
        ...
        if self.weight_var == list():
            ...
            node.y_mean = (self.df.loc[node.idx, self.tgt] *
                           self.df.loc[node.idx, self.weight_var]).sum() / divisor
        ...
        if top_var_is_singlet == False:
            ...
            if role_a in ['n', 'S'] and role_b in ['n', 'S']:
                ...
                for col, cut in zip(top_interact_pair, [cut_a, cut_b]):
                    ...
                    left_sse = (
                        (self.df.loc[left_idx, self.tgt] - self.df.loc[left_idx, self.tgt].mean())**2).sum()
                    right_sse = (
                        (self.df.loc[right_idx, self.tgt] - self.df.loc[right_idx, self.tgt].mean())**2).sum()
                    node_sse = (
                        (self.df.loc[node.idx, self.tgt] - self.df.loc[node.idx, self.tgt].mean())**2).sum()
                    p = 1, len(left_idx) / \
        ...
        if best_var == None:
            logger.log(
                level=logging.INFO, msg=f"file with bad best_var = {self.settings.overwrite_data_text}")
            pdb.set_trace()
    ...
    def fit(self):
        ...
        while len(stack) > 0:
            ...
            curr.node_data = InternalData(split_var=split_var, split_point=split_point,
            ...
            right_node = Node(node_data=None, depth=curr.depth + 1, parent=curr,
                              indices=right, node_num=self._get_next_node_num())
            curr.left = left_node
    ...
    def predict_train_data(self, print_me=False) -> pd.DataFrame:
        ...
        for idx, row in self.df.iterrows():
            ...
            df2 = pd.DataFrame({'train': train, 'node': node,
                                'observed': observed, 'predicted': predicted}, index=[idx])
